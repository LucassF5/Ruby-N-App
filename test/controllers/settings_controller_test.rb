...
class SettingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "updates name and email" do
    patch settings_url, params: { user: { name: "Jane Updated", email_address: "jane.updated@example.com", current_password: "password" } }
  ...
  test "rejects email change without current password" do
    patch settings_url, params: { user: { email_address: "jane.updated@example.com" } }
  ...
  test "rejects email change with wrong current password" do
    patch settings_url, params: { user: { email_address: "jane.updated@example.com", current_password: "wrong" } }
    assert_response :unprocessable_entity
    assert_equal "jane@example.com", users(:jane).reload.email_address
  end
  ...
  test "destroys other sessions after password change" do
    ...
    other_session.get edit_settings_url
